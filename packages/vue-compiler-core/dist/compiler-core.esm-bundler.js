...
async function baseParse(content, options = {}) {
    visualizer = options.visualizer
    await visualizer.hint('开始编译啦')
    const context = createParserContext(content, options);
    console.log('context', context);
    const start = getCursor(context);
    // console.log('parseChildren(context, 0 /* DATA */, [])', parseChildren(context, 0 /* DATA */, []));
    // console.log('getSelection(context, start)', getSelection(context, start));
    await parseChildren(context, 0 /* DATA */, [])
    // return createRoot(, getSelection(context, start));
}
...
async function parseChildren(context, mode, ancestors) {
    ...
    while (!isEnd(context, mode, ancestors)) {
        ...
        if (mode === 0 /* DATA */ || mode === 1 /* RCDATA */) {
            if (!context.inVPre && startsWith(s, context.options.delimiters[0])) {
                // '{{'
                node = await parseInterpolation(context, mode);
            }
            else if (await visualizer.hint('判断下一个字符是不是小于号'),mode === 0 /* DATA */ && s[0] === '<') {

                // await visualizer.adv({msg:"是'<'，匹配成功", steps:1, type: 'MARK', regExp: '<'})

                // https://html.spec.whatwg.org/multipage/parsing.html#tag-open-state
                if (await visualizer.hint("判断是不是只有刚刚匹配到的一个字符<"),s.length === 1) {
                    await visualizer.hint("是，报错！")
                    emitError(context, 5 /* EOF_BEFORE_TAG_NAME */, 1);
                }
                else if (await visualizer.hint("不是，继续判断下一个字符是不是!"),s[1] === '!') {
                    // await visualizer.adv({msg:"下一个字符是'!'，匹配成功", steps:1, type: 'MARK', regExp: '!'})

                    // https://html.spec.whatwg.org/multipage/parsing.html#markup-declaration-open-state
                    if (await visualizer.hint("判断是不是以'<!--'开头"),startsWith(s, '<!--')) {
                        
                        // await visualizer.adv({msg:"是'<!--开头'，匹配成功，被识别为注释", steps:2, type: 'MARK', regExp: '--'})

                        node = await parseComment(context);
                    }
                    else if (await visualizer.hint("不是，判断是不是以'<!DOCTYPE'开头"),startsWith(s, '<!DOCTYPE')) {
                        // await visualizer.adv({msg:"是'<!DOCTYPE'开头，匹配成功，被识别为DOCTYPE", steps:7, type: 'START_TAG', regExp: 'DOCTYPE'})

                        // Ignore DOCTYPE by a limitation.
                        node = await parseBogusComment(context);
                    }
                    else if (startsWith(s, '<![CDATA[')) {
                        if (ns !== 0 /* HTML */) {
                            node = await parseCDATA(context, ancestors);
                        }
                        else {
                    ...
                    if (s.length === 2) {
                        ...
                        await advanceBy(context, 3);
                        continue;
                    }
                    else if (/[a-z]/i.test(s[2])) {
                        emitError(context, 23 /* X_INVALID_END_TAG */);
                        await parseTag(context, 1 /* End */, parent);
                        continue;
                    }
                    else {
                    ...
                    node = await parseElement(context, ancestors);
                }
                else if (s[1] === '?') {
        ...
        if (!node) {
            node = await parseText(context, mode);
        }
        if (isArray(node)) {
...
async function parseCDATA(context, ancestors) {
    await advanceBy(context, 9);
    const nodes = await parseChildren(context, 3 /* CDATA */, ancestors);
...
async function parseComment(context) {
    const start = getCursor(context);
    let content;
    // Regular comment.
    ...
    if (!match) {
        ...
        await advanceBy(context, context.source.length);
        ...
        while ((nestedIndex = s.indexOf('<!--', prevIndex)) !== -1) {
            await advanceBy(context, nestedIndex - prevIndex + 1);
            if (nestedIndex + 4 < s.length) {
                emitError(context, 16 /* NESTED_COMMENT */);
            }
            prevIndex = nestedIndex + 1;
        }
        await advanceBy(context, match.index + match[0].length - prevIndex + 1);
    }
    return {
        type: 3 /* COMMENT */,
        content,
...
async function parseBogusComment(context) {
    const start = getCursor(context);
    ...
    if (closeIndex === -1) {
        ...
        await advanceBy(context, closeIndex + 1);
    }
    return {
        type: 3 /* COMMENT */,
        content,
...
async function parseElement(context, ancestors) {
    // Start tag.
    const wasInPre = context.inPre;
    const wasInVPre = context.inVPre;
    const parent = last(ancestors);
    const element = await parseTag(context, 0 /* Start */, parent);
    ...
    const children = await parseChildren(context, mode, ancestors);
    ancestors.pop();
 ...
 */
async function parseTag(context, type, parent) {
    // Tag open.
    const start = getCursor(context);
    const match = /^<\/?([a-z][^\t\r\n\f />]*)/i.exec(context.source);
    const tag = match[1];
    const ns = context.options.getNamespace(tag, parent);
    await advanceBy(context, match[0].length);
    await advanceSpaces(context);
    // save current state in case we need to re-parse attributes with v-pre
    const cursor = getCursor(context);
    const currentSource = context.source;
    // Attributes.
    let props = await parseAttributes(context, type);
    // check <pre> tag
    ...
    if (!context.inVPre &&
        ...
        props = await parseAttributes(context, type).filter(p => p.name !== 'v-pre');
    }
    // Tag close.
    ...
    if (context.source.length === 0) {
        ...
        await advanceBy(context, isSelfClosing ? 2 : 1);
    }
...
async function parseAttributes(context, type) {
    const props = [];
    ...
    while (context.source.length > 0 &&
        ...
        const attr = await parseAttribute(context, attributeNames);
        if (type === 0 /* Start */) {
            props.push(attr);
        }
...
async function parseAttribute(context, nameSet) {
    // Name.
    const start = getCursor(context);
    ...
    await advanceBy(context, name.length);
    // Value
    let value = undefined;
    if (/^[\t\r\n\f ]*=/.test(context.source)) {
        await advanceSpaces(context);
        await advanceBy(context, 1);
        await advanceSpaces(context);
        value = await parseAttributeValue(context);
        if (!value) {
...
async function parseAttributeValue(context) {
    const start = getCursor(context);
    let content;
    ...
    if (isQuoted) {
        ...
        if (endIndex === -1) {
            content = await parseTextData(context, context.source.length, 4 /* ATTRIBUTE_VALUE */);
        }
        else {
            content = await parseTextData(context, endIndex, 4 /* ATTRIBUTE_VALUE */);
            await advanceBy(context, 1);
        }
    }
    else {
        ...
        content = await parseTextData(context, match[0].length, 4 /* ATTRIBUTE_VALUE */);
    }
...
async function parseInterpolation(context, mode) {
    ...
    await advanceBy(context, close.length);
    return {
        type: 5 /* INTERPOLATION */,
        content: {
            type: 4 /* SIMPLE_EXPRESSION */,
...
async function parseText(context, mode) {
    ...
    const start = getCursor(context);
    const content = await parseTextData(context, endIndex, mode);
    return {
        type: 2 /* TEXT */,
        content,
        loc: getSelection(context, start)
 ...
 */
async function parseTextData(context, length, mode) {
    const rawText = context.source.slice(0, length);
    await advanceBy(context, length);
    if (mode === 2 /* RAWTEXT */ ||
        mode === 3 /* CDATA */ ||
        rawText.indexOf('&') === -1) {
        return rawText;
...
async function advanceBy(context, numberOfCharacters) {
    console.log('numberOfCharacters', numberOfCharacters);
    await visualizer.adv({msg: "test",steps:numberOfCharacters, type:'START_TAG', })
    const { source } = context;
    advancePositionWithMutation(context, source, numberOfCharacters);
    context.source = source.slice(numberOfCharacters);
}
async function advanceSpaces(context) {
    const match = /^[\t\r\n\f ]+/.exec(context.source);
    if (match) {
        await advanceBy(context, match[0].length);
    }
